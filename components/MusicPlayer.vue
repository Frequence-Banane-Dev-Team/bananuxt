        ...
        <DrawerRoot v-model:open="open">
            <DrawerPortal>
                ...
                <DrawerContent
                    ...
                    <div
                        class="p-4 bg-gradient-to-b to-background from-primary-foreground rounded-t-[10px] flex-1 pb-16">
                        ...
                        <div class="flex  flex-col  max-w-80 mx-auto">
                            <img v-if="currentTrack?.image"
                                class="rounded-sm shadow-2xl aspect-square object-cover w-full"
                                :src="currentTrack?.image?.url" />
                            <img v-else-if="currentEmission?.image"
                                class="rounded-sm shadow-2xl aspect-square object-cover w-full"
            ...
            'hidden': currentTrack?.isLive || !isLoaded
                                        ...
                                        <div ref="seekerContainerMobile" @click="updateSeekerContainer($event)"
                                            ...
                                            <input v-model="range" @change="updateSeeker" ref="seekerMobile"
                                                type="range" class="
                                                absolute
                                                rounded-full
                                                my-2
                ...
                <div class="flex items-center justify-between w-full md:w-1/4">
                    <div class="flex items-center justify-start w-3/4 md:w-full pl-4" @click="toggleDrawer()">
                        <NuxtLink :to="trackLink">
                            <img v-if="currentTrack?.image"
                                class="rounded-sm shadow-2xl aspect-square object-cover w-14 md:w-16"
                                :src="currentTrack?.image?.url" />
                            <img v-else-if="currentEmission?.image"
                                class="rounded-sm shadow-2xl aspect-square object-cover w-14 md:w-16"
                                :src="currentEmission?.image?.url" />
                        ...
                        <div class="pl-4 flex flex-col items-start text-start">
                            <div v-if="currentTrack" class="flex flex-wrap text-[14px] text-primary">
                                <NuxtLink class="hidden md:block hover:underline cursor-pointer"
                                    v-if="!currentTrack.isLive" :to="trackLink">
                                    {{ currentTrack?.title }}
                                </NuxtLink>
                    ...
                    <div
                        class="buttons flex md:hidden items-center justify-end pr-5 md:pr-4 gap-3 h-[25px] md:h-[30px] w-1/4">
                        <span>
            ...
            'md:pt-2': !currentTrack?.isLive
                            ...
                            <span v-if="!currentTrack?.isLive" class="mx-2">
                                <Rewind15 v-if="!currentTrack?.isLive" class="text-primary hover:cursor-pointer"
                                    :size="25" @click="useSong.rewindCurrentSong(15)" />
